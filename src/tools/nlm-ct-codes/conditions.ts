import { CodeResult, ConditionsMethodArgs } from './types.js';
import { makeApiRequest, validateAndProcessAdditionalQuery } from './utils.js';

/**
 * Search medical conditions from the Regenstrief Institute's Medical Gopher program
 * Contains over 2,400 medical conditions with ICD-10-CM codes, ICD-9-CM codes, and associated terms
 */
export async function searchConditions(args: ConditionsMethodArgs): Promise<CodeResult[]> {
  const {
    terms,
    maxList,
    count,
  ...
  // Conditions API returns array format: [total, codes, extraData, displayData, codeSystems]
  if (!Array.isArray(response) || response.length < 4) {
    throw new Error('Invalid response format from Conditions API');
  }

  const [, codes, extraData, displayData] = response;

  if (!Array.isArray(codes) || !Array.isArray(displayData)) {
    throw new Error('Invalid response structure from Conditions API');
  }
