// NPI (National Provider Identifier) Individuals implementation

import { NpiIndividualsMethodArgs, SearchResponse, CodeResult } from './types.js';
import {
  validateTerms,
  validateMaxList,
  validateOffset,
...
export async function searchNpiIndividuals(
  args: NpiIndividualsMethodArgs
  ...
  // Set defaults for NPI Individuals
  ...
  // Build API URL (note: npi_idv for individuals, not npi_org)
  ...
  const apiUrl = `https://clinicaltables.nlm.nih.gov/api/npi_idv/v3/search?${params.toString()}`;

  try {
    const [totalCount, codes, extraData, displayStrings] = await makeApiRequest(apiUrl);

    // Transform the results into NPI Individuals format
    ...
    if (Array.isArray(codes) && Array.isArray(displayStrings)) {
      for (let i = 0; i < codes.length; i++) {
        ...
        if (extraData) {
          // Gender field (specific to individuals)
          if (extraData['gender']?.[i]) {
            result.gender = extraData['gender'][i];
          }
    ...
    if (error instanceof Error) {
      throw new Error(`Failed to search NPI Individual records: ${error.message}`);
    }
    throw new Error('Failed to search NPI Individual records: Unknown error');
  }
}
